...
metadata, attachments, and a SQLite database in one portable file. Its primary
table model is a document-native Formula sheet: a loose `Any` grid can be
edited first, then progressively constrained as text, number, or boolean data,
with individual cells optionally becoming bounded formulas. Sandboxed Rhai
scripts can consume Formula tables and publish read-only derived results.
Legacy query-backed Formula definitions remain readable for compatibility.
...
Rust renderer for attachments, managed Formula sheets, and sandboxed
